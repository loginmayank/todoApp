...
export class UserService {
  ...
  getCurrentUser(): Observable<User> {
    return this.http.get('http://localhost:3000/api/current-user', { headers: this.headers }).catch(err => {
      return Observable.throw(err);
});
  }
  ...
  logOut(): Observable<User> {
    ...
    return this.http.post('http://localhost:3000/api/log-out', {}, { headers: this.headers })
    .map(res => res as User)
    .catch(err => {
      return Observable.throw(err);
});
